...
def test_shannon_nyquist_warning() -> None:
    ...
    The Gaussian generator (FWHM = 8 ns) has a maximum significant
    frequency of ~1.7e8 Hz; sampling at dt = 1e-7 s gives a Nyquist
    frequency of only 5e6 Hz, so the constructor must warn about
    under-sampling.
    """
    generator = GaussianGenerator(
        height=1.0, mean=30e-9, FWHM=8e-9, R_g=10.0, C_g=0.0
    ...
    load = ConstantResistance(R=150.0)
    cable = PerfectCable(L=1.0, Z_c=50.0, c=2.0e8)
    with pytest.warns(UserWarning, match="under-samples the generator"):
        # dt = 1e-6 / 10 = 1e-7 s; Nyquist 5 MHz << f_max ~ 174 MHz.
        SteadyImpedanceSolution(
